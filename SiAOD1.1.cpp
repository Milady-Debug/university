...
// Преобразование строки в массив символов (сырой указатель)
char* stringToCharArray(const string& str, int& len) {
    len = str.length();
...
void printArr(char* arr, int N, const string& name = "") {
...
void executeTask1() {
    ...
    // Освобождение памяти
    delete[] arr;
    delete[] arrayFirstAlg;
    delete[] arraySecondAlg;
}
